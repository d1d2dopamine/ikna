...
| 6. CI-контракт строки колоды | `602d40b` | `MobileDeckUiContractTest` требовал прямое соседство `.height(DECK_ROW_HEIGHT)`/`.clipToBounds()` — подстрока отсутствует уже в initial-коммите (IKNA-T-004, S2, найдено по логу CI владельца); требуемая цепочка актуализирована до текущей, смысл контракта сохранён | скрипт-проверка 10/10 подстрок; перезапуск CI — за владельцем |
| 7. Документация планов | `7977b8d` | Track C/J чекбоксы modern_PLAN актуализированы; в Track J записано решение владельца о двух видах интерфейса (вариант 1 — угловатый, вариант 2 — скруглённый, одно дерево компонентов); одностраничный статус 0.11 | check_text exit 0 |
| 8. Научная база и аудиты | `7870d41`, `f33e556` | `docs/rele.md` — ≈30 верифицированных работ и ресурсов в структуре «фундамент + 10 возможностей улучшения» (риджелайн Cepeda 2008, successive relearning, pretesting, компьютерный фидбек, hypercorrection, overlearning, морфология, если-то напоминания); `docs/ai-audits.md` — 5 аудитов (usability-карта Track G, инвентаризация углов, a11y, вердикты по дубликатам, отложенный долг) | check_text exit 0 |
| 9. Симуляция выдачи карточек | этот коммит | `LearningSimulationTest` (desktop test-sourceset): 30-дневные симуляции учеников через реальный план/governor/FSRS с явными часами; см. раздел «Симуляционные эксперименты» ниже | `:desktop:test` 100/0 (12 наборов) |

## Симуляционные эксперименты выдачи карточек (после прогона)

**Ключевое техническое открытие:** часы планировщика — параметр
(`ensureDailyPlan(now)`, `answer(..., now)`, `DeveloperSandboxSeeder.seed(now)`),
поэтому симуляция 5–30 дней обучения делается без подмены системных часов:
`now` двигается днями от фиксированного якоря (2026-09-01 19:00), ответы
скриптованного ученика идут с шагом 45 секунд внутри дня. Харнесс —
`desktop/src/test/kotlin/dev/ikna/desktop/LearningSimulationTest.kt` (test-sourceset,
продукт-код не тронут), запускается на одноразовом dev-профиле во временной
папке, как и существующий `DeveloperSandboxIntegrationTest`.

**Инцидент с ресурсами (честно):** первый прогон трёх 30-дневных симуляций
совпал с исчерпанием памяти на машине владельца (ноут завис намертво, аварийное
выключение). Причина — совокупная память нескольких fork-нутых JVM (Gradle
single-use daemon, Kotlin compile daemon, тест-JVM) без капов. В скиллы
(`ikna-testing`, `ikna-development`) вписано правило ресурсной безопасности:
один Gradle-запуск за раз, капы `GRADLE_OPTS`/`-Dorg.gradle.jvmargs=-Xmx768m`
и `-Dkotlin.daemon.jvm.options=-Xmx512m`, очистка висячих JVM после каждого
тяжёлого прогона, проверка свободной памяти до старта.

**Эксперименты (все — `:desktop:test`, итог: 12 наборов, 100 тестов, 0 падений):**

| Эксперимент | Сценарий | Инварианты |
| --- | --- | --- |
| 30 дней, прилежный ученик | `EARLY_HISTORY` + 30 дней GOOD/HARD | план ≤ capacity ≤ backlogHardLimit каждый день; журнал append-only (`seeded + answered == reviews.total`); план идемпотентен в течение дня |
| 30 дней, испытывающий трудности (40% AGAIN) | `EARLY_HISTORY` | те же лимиты при серии провалов; append-only сохраняется |
| 10 дней, детерминизм | `EARLY_HISTORY`, один сид | два независимых прогона дают идентичные планы по дням |
| Перерыв 14 дней → возврат | `MATURE_HISTORY` | первый день после перерыва: причина `SAFETY_VALVE` или `RETURN_MODE`, план ≤ `returnModeCapacity` (10), ≤ 1 новой карточки |
| План в 23:30 (ночной cutoff) | `EARLY_HISTORY` | `allowedNew == 0` при работающих повторах; следующий вечер — не ночной |

**Наблюдения из 30-дневных таблиц** (полные таблицы — артефакт
`desktop/build/sim-*.txt`):

```
diligent 30 дней:   d01 planned=13 (POST_SKIP_WARMUP, new=0) → d02 NO_HEADROOM →
                    d03+ OK, введения зарабатываются: new=0 → 1 → 2, план 4-13/день
struggling 30 дней: d01 planned=13 (7 AGAIN) → d03-d06 LOW_ACCURACY (new=0 —
                    новый материал остановлен при точности ниже 0.75) →
                    d07 SAFETY_VALVE (new=1) → capacity 14 → 13, план 10-14/день
```

**Выводы о корректности выдачи карточек:**

1. Размер плана ограничен governor'ом при любом поведении: 13→4 у прилежного,
   10–14 у испытывающего трудности, жёсткий лимит 120 не достигается ни разу.
2. Введения нового материала зарабатываются вниманием (0 → 1 → 2 за первую
   неделю прилежного), а при низкой точности останавливаются (`LOW_ACCURACY`,
   d03–d06) — ровно как сформулировано в GovernorConfig.
3. Safety valve срабатывает редко и мягко (1 из 30 дней, new=1) и корректно
   перекрывает return-mode на длинном перерыве, сохраняя исходный gate в
   `governor_log.gate`.
4. Журнал отзывов append-only на всей симуляции: `seeded + answered == total`
   без единого расхождения.
5. Два прогона одного сценария дают побитово одинаковые планы — детерминизм
   сидера и планировщика подтверждён.

Расхождений с ожидаемой выдачей не найдено; единственная поправка —
документированные приоритеты причин governor'а (safety valve перекрывает
return mode; post-skip warmup перекрывает late night) зафиксированы в тесте
как контракт.

### Анатомия одного дня: что именно делало приложение

Разбор дня d01 прилежного ученика (числа реальные, из таблицы выше). День =
19:00 якорного числа + 1 сутки; граница суток в ikna — 04:00, поэтому 19:00 и
23:30 одного календарного дня — это один «учебный день».

1. **`ensureDailyPlan(19:00)`** — берётся блокировка записи, вычисляется ключ
   дня, проверяется: план на этот день уже сохранён? Нет — день новый.
2. **Измеренная норма.** `autoTarget()`: медиана отвеченных карточек на
   активных днях за 14 дней (сеяно 4 дня × 12 = по 12 в день) × надбавка,
   минимум 12 (`AUTO_MIN`) → **capacity = 13**. Это не «40 из конфига»:
   40 — потолок, а дневная норма — измеренная по фактическому поведению
   (медиана, а не среднее — один героический вечер не становится стандартом).
3. **Амнистия.** Карточки, просроченные более чем на 2 дня
   (`amnestyAfterDays`), уходят из видимой очереди в пул амнистии — они не
   прощены, FSRS пересчитает их устойчивость по реальному времени.
4. **Сигналы в governor**: `dueToday` (сколько карточек достигло FSRS-срока —
   13), `backlog` (размер амнистии), прогноз выдачи на 3 дня, точность
   недавних ответов, дней с последней сессии, всего отзывов.
5. **Решение governor'а.** Гейты проверяются по порядку: cold start →
   return mode → перегрев → долг > 120 → низкая активность → **post-skip
   warmup** (прошлый день без ответов + меньше 10 ответов сегодня → нового
   материала 0) → низкая точность. На d01 сработал post-skip warmup:
   `allowedNew = 0`. Затем night rule: 19:00 — не ночь, и правилу нечего
   менять (новый материал уже 0 — ранний выход).
6. **Журнал решения.** В `governor_log` пишется всё, из чего состояло решение
   (сигналы, capacity, allowedNew, причина, gate) — колонками, а не строкой.
7. **Введения.** `introduce(roomNew)` — единственное место, где создаются
   новые карточки; неразмещенные в плане — удаляются, считаются только
   попавшие. На d01 roomNew=0 → введений нет.
8. **Сборка плана.** `pickForDay` отбирает due-карточки плюс введённые,
   убирает подавленные и уже отвеченные сегодня; план сохраняется
   (`storePlan`) — второй вызов `ensureDailyPlan` в тот же день возвращает
   **тот же план** (идемпотентность, проверена тестом).
9. **Сессия.** `buildSession` = план − отвеченное сегодня = очередь из 13.
10. **Ответы.** 13 × `answer(карточка, оценка, now+45с×i)`: FSRS применяет
    оценку, пересчитывает устойчивость/сложность и срок следующего показа; в
    строку отзыва пишется снапшот параметров FSRS на момент ответа.

### Почему цифры — эталон этой версии

| Наблюдение | Число | Почему это правильно (наука + конфиг) |
| --- | --- | --- |
| capacity 13 на старте | медиана 12 × надбавка, `AUTO_MIN=12` | норма измерена по поведению, а не назначена; «40» из конфига — потолок, не дефолт |
| d02 `NO_HEADROOM`, new=0 | due-повторы заняли все 13 мест | «сначала долги, потом новое» — прямое следствие распределённой практики (Cepeda 2006) |
| d03 `OK`, new=1 | headroom = 13 − 4 due = 9 ≥ `costPerNew` (4) | новая карточка «стоит» ~4 повторений на неделе вперёд — цена платится, а не одалживается |
| введения 0 → 1 → 2 | рост только после чистых дней | новый материал не бесплатен: «лавина через три недели — то, что убивает привычку» (LoadGovernor.kt) |
| struggling: `LOW_ACCURACY` с d03 по d30 (new=0) | точность < 0.75 (`minAccuracy`) | добавлять куски в проваливающуюся неделю — «как строится куча, убивающая привычку» |
| struggling: `SAFETY_VALVE` на d15 и d23 (new=1) | клапан: 1 кусок за `safetyValveDays=7` | без клапана governor «застревает в нуле» и приложение становится беговой дорожкой с закрытыми дверями |
| план 10–14 при серии провалов | не 0 и не 120 | карточки с AGAIN возвращаются раньше (FSRS укорачивает интервал) — очередь живая, но ограниченная |
| return после 14 дней: план 3, capacity 10, new=1 | `returnModeGapDays=14`, `returnModeCapacity=10` | возвращающийся получает маленький день, а не стену; клапан перекрыл return-гейт, сохранив его в логе |
| ночь 23:30: new=0, повторять можно | `nightCutoffHour=23`, `dayStartHour=4` | отсроченная фаза сна типична для ADHD-аудитории; 01:30 — вечерняя сессия, а не утренняя |

### Почему эталон не нарушен — проверенные инварианты

Каждая строка — не наблюдение глазами, а assert в `LearningSimulationTest`:

| Инвариант | Как проверено | Результат за 30 дней |
| --- | --- | --- |
| План ≤ capacity ≤ жёсткий лимит 120 | assert на каждый день, обе персоны | 30/30 и 30/30 дней |
| Ни одной карточки дважды в день | множество ключей == размеру очереди | 30/30 дней |
| Журнал отзывов append-only | `seeded(48) + answered(178) == total(226)`; `48 + 329 == 377` | точное равенство, оба прогона |
| Идемпотентность плана | второй `ensureDailyPlan` в тот же день | тот же plannedTotal и reason |
| Детерминизм | два прогона с одним сидом | идентичные (день, план, причина) |
| Валидные причины | reason ∈ GovernorReason каждый день | 30/30 дней |
| Ночной контракт | allowedNew == 0 в 23:30, повторами не мешает | выполнено |
| Возвратный контракт | план ≤ 10, новых ≤ 1 | выполнено |

### Как воспроизвести (для другого ИИ или человека)

```bash
set JAVA_HOME=C:\Users\work\AppData\Local\Programs\Microsoft\jdk-17.0.20.101-hotspot
set GRADLE_OPTS=-Dorg.gradle.jvmargs=-Xmx768m -Dkotlin.daemon.jvm.options=-Xmx512m
cd C:\Users\work\.zcode\workspace\default\ikna
C:\Users\work\.zcode\tools\gradle-8.10.2\bin\gradle.bat --no-daemon ^
  -Dorg.gradle.jvmargs=-Xmx768m -Dkotlin.daemon.jvm.options=-Xmx512m ^
  :desktop:test --tests dev.ikna.desktop.LearningSimulationTest
```

Код харнесса: `desktop/src/test/kotlin/dev/ikna/desktop/LearningSimulationTest.kt`.
Читать вместе: `LoadGovernor.kt` (решение и гейты), `LearningRepository.kt:204-343`
(план дня), `GovernorConfig.kt` (все load-bearing числа), `DayBoundary.kt`
(граница 04:00). Таблицы дней пишутся в `desktop/build/sim-*.txt`. Правило
ресурсной безопасности (один запуск Gradle за раз, капы памяти) — обязательно.
