...
- [x] add an isolated Developer Sandbox with separate data/settings, deterministic
  synthetic learner scenarios, persistent DEV indication and automatic forced access
  that bypasses product restrictions while preserving production policy verdicts;
- [ ] verify Developer Sandbox profile switching, reseed and automatic forced Browse access on
...
- [x] reshape Browse into a passive vertical reading feed: complete items stack one after another, scrolling replaces swipe-to-advance, and exposure is recorded only for meaningfully visible items;
