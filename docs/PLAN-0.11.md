...
**Implementation status:** census tooling is implemented and the first full 11-language evidence run completed successfully on 2026-09-14. Review of that artifact found one reporting bug: an acquisition-capped WikiMatrix pair could be labelled `deck-cap-truncated` before `source-scan-lower-bound`, hiding the fact that the source measurement was incomplete. The classifier now gives the acquisition bound precedence. A 2026-09-20 rerun at 200,000 rows exposed a second streaming-verifier bug: a legitimate score-threshold stop below `max_rows` closed the upstream pipe and was mistaken for a truncated download. The stream now writes an explicit reader stop-reason sidecar, accepts only verified `max-rows` or `score-threshold` closes, and still fails closed when an upstream error ends at plain EOF. The reviewed 200,000-row evidence artifact uses the 2026-09-19 Tatoeba snapshot and covers all 55 physical WikiMatrix pairs: 19 terminate cleanly at the score threshold, while 36 still hit `max-rows`, leaving 72 directed Knowledge measurements as lower bounds. The workflow now computes global Knowledge ranks in language shards and measures physical WikiMatrix pairs independently; raw candidate JSONL never leaves the runner that produced it. `test_supply_census_shards.py` requires the sharded report to match the original monolithic `supply_census.py` report exactly on a multi-pair fixture, so the execution shape can change without changing supply/selection semantics. A larger evidence bound therefore no longer forces all 55 source streams through one six-hour runner. The manual workflow default is 500,000 rows for the next continuation run; repeat with a larger bound for any pair that still reaches it. Part 5 remains open until no planned Knowledge pair is reported as `source-scan-lower-bound`. The workflow never publishes assets.
