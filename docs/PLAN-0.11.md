...
**Implementation status:** census tooling is implemented and the first full 11-language evidence run completed successfully on 2026-09-14. Review of that artifact found one reporting bug: an acquisition-capped WikiMatrix pair could be labelled `deck-cap-truncated` before `source-scan-lower-bound`, hiding the fact that the source measurement was incomplete. The classifier now gives the acquisition bound precedence. A 2026-09-20 rerun at 200,000 rows exposed a second streaming-verifier bug: a legitimate score-threshold stop below `max_rows` closed the upstream pipe and was mistaken for a truncated download. The stream now writes an explicit reader stop-reason sidecar, accepts only verified `max-rows` or `score-threshold` closes, and still fails closed when an upstream error ends at plain EOF. The 2026-09-23 sharded continuation run at 500,000 rows used the same 2026-09-19 Tatoeba snapshot and completed successfully across all 55 physical WikiMatrix pairs: 40 reached the score threshold, 15 still hit the artificial row bound, no direct source file was missing, and 30 directed Knowledge measurements therefore remained lower bounds. That run confirmed the sharded execution path on real scale but also showed that increasing the bound stepwise is unnecessary: WikiMatrix v1 rows are score-sorted and the reader already stops naturally at the configured score floor. The workflow now uses an uncapped (`wikimatrix_max_rows = 0`) full scan as the normal Part 5 evidence mode; non-zero row bounds remain only for debugging/short diagnostics. Rank acquisition is sharded by physical pair, reduces both learning directions to compact token-count shards in one local pass, then aggregates global ranks before a second and final physical-pair stream performs measurement. Raw candidate JSONL never crosses jobs. `test_supply_census_shards.py` requires this sharded path to match the original monolithic `supply_census.py` report exactly on fixtures. Part 5 remains open until one uncapped run reports no planned Knowledge pair as `source-scan-lower-bound`. The workflow never publishes assets.
