...
- [ ] collect any further small app bugs found while testing the current build;
  - 2026-09-22: desktop window-state cleanup now keeps floating geometry separate from maximized/fullscreen bounds, disables title-bar dragging while maximized, and makes F11 restore the previous non-fullscreen placement; focused regression/static checks pass, while a real Windows GUI pass is still required.
