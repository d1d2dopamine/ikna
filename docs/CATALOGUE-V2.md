...
WikiMatrix availability is measured separately from target supply. A missing direct upstream file is reported as such. WikiMatrix v1 TSVs are score-sorted, so the normal Part 5 evidence mode streams each physical pair without a row cap and stops naturally when the alignment score falls below the configured floor (or at clean EOF). `wikimatrix_max_rows` remains an optional diagnostic/debug bound only; any non-zero bound that is reached produces `source-scan-lower-bound` and cannot close Part 5. Global Knowledge ranks require one complete pass over the bounded-by-score candidate space before pair selection can be measured, so the sharded workflow intentionally performs two upstream streams per physical pair: one compact rank-reduction pass and one measurement pass. Raw candidate JSONL stays on the runner that produced it. The workflow is `.github/workflows/catalogue-v2-supply-census.yml`; report semantics live in `tools/catalog/supply_census.py`, while `tools/catalog/supply_census_shards.py` only changes execution shape. It has no publication step.
