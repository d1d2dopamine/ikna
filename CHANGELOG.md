...
- Fixed the desktop compilation blocker: three `IknaChip` calls in `SettingsPane.kt` passed the click handler as a trailing lambda that landed in the `modifier` parameter; named `onClick` arguments restore the build. The snapshot's own contract test for the deck row was stale in the same way and now asserts the current modifier chain (`MobileDeckUiContractTest`, recorded as IKNA-T-004).
- Closed the platform-font gaps in the shared typography: `headlineLarge`, `titleLarge` and `titleSmall` now exist in the base typography and follow the selected main face, so Browse prompts, deck/voice subtitles and the Anki import header stop falling back to the system font. The system-font appearance mode (`FontMode.SYSTEM`) is retired — its chips are gone from Settings on both platforms, and stored settings and backups that named it migrate to Geologica.
- Settled the Browse reading measure: the feed column is capped at 640dp on desktop and Android, long prompts read at a 26sp leading that matches the app's prose rhythm, and the provenance block is separated by its own gap.
- The memory lattice now breathes in the window chrome: `IknaMemoryAmbientStrip` repaints the deck-header ladder on the title bar and the bottom bar, where a few marks dissolve while others appear on one deterministic 36-second cycle. With animations off the strips are exactly the static field. The signal frame keeps its tuned motion — a bordered control now shows keyboard focus through its own boundary instead of the root overlay.
