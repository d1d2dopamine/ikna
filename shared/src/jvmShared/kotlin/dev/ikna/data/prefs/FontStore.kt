...
object FontStore {
    ...
    /**
     * Where installed fonts live. Set once at startup -- filesDir on Android,
     * the per-user application data folder on Windows -- because a font store
     * that asks for a Context on every call cannot be shared by two
     * applications, and this object is reached from the theme, which has no
     * business knowing about either platform.
     */
    @Volatile
    var baseDir: File? = null

    private fun dir(): File = baseDir ?: File(".")
     ...
     */
    fun install(input: InputStream): String? {
