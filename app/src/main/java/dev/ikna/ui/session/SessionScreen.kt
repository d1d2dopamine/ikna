...
/**
 * One line of service text, and most of the time it is empty.
 *
 * It used to carry a live "осталось 12 · ~3 мин" on the left and "дальше:
 * вставить" on the right, next to the card, for the whole session. Both are
 * answers to questions nobody asks in the middle of recalling something, and a
 * counter that ticks down beside the thing you are reading is an invitation to
 * watch the counter instead.
 *
 * So the cost of the session is shown once, before the first answer, where it is
 * a decision — and then the line goes quiet. The one thing that earns a word
 * back is the minimum being reached, which is shown at the moment it happens and
 * for one card only.
 */
@Composable
    ...
    Box(
        ...
        if (text.isNotEmpty()) {
            Text(
                text = text,
                style = MaterialTheme.typography.labelMedium,
                color = if (minimumJustMet) MaterialTheme.colorScheme.primary else IknaMuted,
                maxLines = 1
            )
        }
    }
    ...
    Box(
        ...
        when {
            ...
            visible -> Row(
                ...
                IknaTextButton(label = "ОК", onClick = onDismiss, color = IknaMuted)
            }
        }
    }
}
